                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        @foreach ( $tables as $position )
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700" id="{{ $search }}_ids{{ $position->id }}">
                                <td class="w-4 p-4">
                                    <div class="flex items-center">
                                        <input id="" aria-describedby="checkbox-1" name="ids" type="checkbox" class="checkbox_ids w-4 h-4 border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-primary-300 dark:focus:ring-primary-600 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600" value="{{ $position->id }}">
                                        <label for="checkbox" class="sr-only">checkbox</label>
                                    </div>
                                </td>
                                <td class="w-full p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $position->name }}</td>
                                <td class="p-4 space-x-2 whitespace-nowrap">
                                    <!-- Edit User Modal -->
                                    @include('position.edit')
                                    @can('delete data')
                                        <form action="/position/{{ $position->id }}" method="POST" class="inline-flex">
                                            @method('delete')
                                            @csrf
                                            <!-- Delete User Modal -->
                                            @include('position.delete')
                                        </form>
                                    @endcan
                                </td>
                            </tr>
