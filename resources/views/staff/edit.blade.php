...
<script>
    $(document).ready(function() {
        ...
        $('.regency-edit-{{ $staff->id }}').prop("disabled", false);
        
        // Populate district
        $('.district-edit-{{ $staff->id }}').empty();
        ...
        $('.district-edit-{{ $staff->id }}').prop("disabled", false);

        // Populate village
        $('.village-edit-{{ $staff->id }}').empty();
        $('.village-edit-{{ $staff->id }}').append(
            '<option value="">Pilih Desa/Kelurahan</option>'
        );
        $.each(villages, function(i, village) {
            $('.village-edit-{{ $staff->id }}').append(
                '<option ' + (village.id == {{ $staff->village_id }} ? 'selected' : '') + ' value="' +
                village.id +
                '">' +
                village.name +
                "</option>"
        ...
        $('.village-edit-{{ $staff->id }}').prop("disabled", false);

        // Make ajax request to get regencies based on selected province
        $('.province-edit-{{ $staff->id }}').on("change", function() {
            $('.regency-edit-{{ $staff->id }}').prop("disabled", true);
            $('.district-edit-{{ $staff->id }}').prop("disabled", true);
            $('.village-edit-{{ $staff->id }}').prop("disabled", true);

            $('.regency-edit-{{ $staff->id }}').html(
                '<option selected value="">Pilih Kabupaten/Kota</option>'
            );
            $('.district-edit-{{ $staff->id }}').html(
                '<option selected value="">Pilih Kecamatan</option>'
            );
            $('.village-edit-{{ $staff->id }}').html(
